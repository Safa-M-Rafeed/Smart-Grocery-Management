...
const createLoyaltyTransaction = async (req, res) => {
    try {
        const { customerID, transactionType = 'CARD_CREATED', pointsEarned = 0, pointsRedeemed = 0 } = req.body;
...
const createPurchaseLoyaltyTransaction = async (req, res) => {
    try {
        ...
        if (customer) {
            customer.loyaltyPoints = customer.loyaltyPoints + pointsEarned - pointsRedeemed;
